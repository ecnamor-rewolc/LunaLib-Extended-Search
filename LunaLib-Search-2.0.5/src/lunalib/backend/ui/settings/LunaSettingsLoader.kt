...
object LunaSettingsLoader
{
    ...
    /**
     * Loads a per-mod LunaSettings JSON file from the common data folder, but tolerates corruption.
     * If the file is unreadable or malformed, it is moved aside (.corrupt-<timestamp>) and a fresh
     * blank config is created. This prevents one bad JSON (from an old mod version, partial write,
     * or an orphaned config left behind by an uninstalled mod) from breaking LunaLib startup.
     */
    private fun loadConfigSafe(modId: String): JSONUtils.CommonDataJSONObject?
    {
        val path = "LunaSettings/${modId}.json"
        try {
            return JSONUtils.loadCommonJSON(path, "data/config/LunaSettingsDefault.default")
        } catch (e: Throwable) {
            log.error("LunaSettings: failed to load config for $modId, attempting recovery", e)
            try {
                val saves = Global.getSettings().getString("saveDir") // usually "saves"
                val full = java.io.File(saves, "common/$path")
                if (full.exists()) {
                    val backup = java.io.File(full.parentFile, "${modId}.json.corrupt-${System.currentTimeMillis()}")
                    full.renameTo(backup)
                    log.error("LunaSettings: moved corrupt config to ${backup.name}")
                }
                return JSONUtils.loadCommonJSON(path, "data/config/LunaSettingsDefault.default")
            } catch (inner: Throwable) {
                log.error("LunaSettings: recovery failed for $modId, skipping", inner)
                return null
            }
        }
    }
    ...
    fun saveDefaultsToFile(modID: String = "")
    {
        ...
        for (mod in mods)
        {
            var data = loadConfigSafe(mod.id) ?: continue
    ...
    fun loadSettings(modID: String = "", reload: Boolean = false)
    {
        ...
        for (mod in mods)
        {
            var data = loadConfigSafe(mod.id)
