...
class LunaSettingsUISettingsPanel() : CustomUIPanelPlugin
{
    ...
    fun recreateTabs()
    {
        ...
        for (row in rows)
        {
            ...
            for (tab in tabs)
            {
                ...
                // A tab is a "match" if ANY whitespace-separated search word is a prefix of some
                // token in the name/id/description/tab fields of the tab's settings (OR across
                // words). The carousel filter is AND across words spread over the whole mod, but
                // for the per-tab gray-out we want any tab that contains at least one match to
                // stay highlightable — otherwise tabs holding only part of a multi-word query
                // end up gray even though the user can clearly see green matches inside them.
                val search = LunaSettingsUIModsPanel.rawSearch
                val tabHasMatch = if (search.isEmpty()) true
                    else {
                        val searchWords = search.split(Regex("\\s+")).filter { it.isNotEmpty() }
                        val tabHaystack = mutableListOf<String>()
                        val exact = LunaSettingsUIModsPanel.exactMatchOnly
                        for (d in LunaSettingsLoader.SettingsData) {
                            if (d.modID != selectedMod!!.id || d.tab != tab) continue
                            tabHaystack.add(d.fieldName)
                            if (!exact) tabHaystack.add(d.fieldID)
                            tabHaystack.add(d.fieldDescription)
                            tabHaystack.add(d.tab)
                        }
                        searchWords.any { word ->
                            tabHaystack.any { LunaSettingsUIModsPanel.anyTokenMatches(it, word) }
                        }
                    }
                ...
                var button = LunaUIButton(false, false, space, tabHeight, "key", "TabsButtons", tabspanel!!, tabspanelElement!!).apply {
                    ...
                    // Highlight each matching word of the tab label independently in the search color (green).
                    if (search.isNotEmpty()) {
                        val tabMatches = LunaSettingsUIModsPanel.extractMatches(name, search)
                        if (tabMatches.isNotEmpty()) {
                            this.buttonText!!.setHighlight(*tabMatches.toTypedArray())
                            this.buttonText!!.setHighlightColor(LunaSettingsUIModsPanel.getSearchHighlightColor())
                        }
                    }
                    if (!tabHasMatch) {
                        this.buttonText!!.setColor(Misc.getGrayColor())
                    }
    ...
    fun recreatePanel()
    {
        ...
        for (data in LunaSettingsLoader.SettingsData) {
            ...
            if (data.fieldType == SettingsType.Header.toString())
            {
                ...
                // Section headers are LabelAPI too, so apply the same per-word search highlight.
                val searchForHeader = LunaSettingsUIModsPanel.rawSearch
                if (searchForHeader.isNotEmpty()) {
                    val headerMatches = LunaSettingsUIModsPanel.extractMatches("${data.defaultValue}", searchForHeader)
                    if (headerMatches.isNotEmpty()) {
                        header.setHighlight(*headerMatches.toTypedArray())
                        header.setHighlightColor(LunaSettingsUIModsPanel.getSearchHighlightColor())
                    }
                }
            ...
            else if (data.fieldType == SettingsType.Text.toString())
            {
                ...
                val bracketHighlights = mutableListOf<String>()
                while (descText.indexOf("[") != -1 && abortAttempts > 0)
                {
                    abortAttempts--
                    bracketHighlights.add(descText.substring(descText.indexOf("[") + 1, descText.indexOf("]")))
                ...
                // Same bracket/search overlap rule as for regular settings: search matches always win
                // and render in the green search color, leaving non-overlapping brackets in yellow.
                val searchForText = LunaSettingsUIModsPanel.rawSearch
                val searchHighlights: List<String> = if (searchForText.isNotEmpty())
                    LunaSettingsUIModsPanel.extractMatches(description.text, searchForText)
                    else emptyList()
                val keptBrackets = if (searchHighlights.isEmpty()) bracketHighlights
                    else bracketHighlights.filterNot { br ->
                        val brLow = br.lowercase()
                        searchHighlights.any { sh ->
                            val shLow = sh.lowercase()
                            brLow.contains(shLow) || shLow.contains(brLow)
                        }
                    }

                // Interleave bracket (yellow) and search (green) highlights, sorted by their first
                // token position in the text. LabelAPI walks the text with a forward-only cursor;
                // out-of-order highlights are silently dropped.
                val entries = mutableListOf<Triple<Int, String, java.awt.Color>>()
                for (br in keptBrackets) {
                    val pos = LunaSettingsUIModsPanel.findFirstTokenPosition(description.text, br)
                    if (pos >= 0) entries.add(Triple(pos, br, Misc.getHighlightColor()))
                }
                for (sh in searchHighlights) {
                    val pos = LunaSettingsUIModsPanel.findFirstTokenPosition(description.text, sh)
                    if (pos >= 0) entries.add(Triple(pos, sh, LunaSettingsUIModsPanel.getSearchHighlightColor()))
                }
                entries.sortBy { it.first }
                if (entries.isNotEmpty()) {
                    description.setHighlight(*entries.map { it.second }.toTypedArray())
                    description.setHighlightColors(*entries.map { it.third }.toTypedArray())
                }
            ...
            else
            {
               ...
               // descriptionElement.setParaSmallInsignia()
                var name = descriptionElement.addPara("${data.fieldName}", 0f, Misc.getBrightPlayerColor(), LunaSettingsUIModsPanel.getSearchHighlightColor())
                // Per-word highlight inside the setting name so partial typing still lights up.
                val searchForName = LunaSettingsUIModsPanel.rawSearch
                if (searchForName.isNotEmpty()) {
                    val nameMatches = LunaSettingsUIModsPanel.extractMatches(data.fieldName, searchForName)
                    if (nameMatches.isNotEmpty()) name.setHighlight(*nameMatches.toTypedArray())
                }
                ...
                // Search-driven per-word matches against the bracket-stripped description text.
                val searchForDesc = LunaSettingsUIModsPanel.rawSearch
                val searchHighlights: List<String> = if (searchForDesc.isNotEmpty())
                    LunaSettingsUIModsPanel.extractMatches(description.text, searchForDesc)
                    else emptyList()

                // LabelAPI.setHighlight consumes the first text occurrence per array entry, so a bracket
                // and a search match pointing at the same substring would collide — the bracket wins
                // (yellow) and the search highlight silently fails to render. To make the search color
                // always win where the user is looking, drop any bracket whose text overlaps with a
                // search-match string (in either direction, case-insensitive). The remaining brackets
                // keep their author-intended yellow.
                val keptBrackets = if (searchHighlights.isEmpty()) bracketHighlights
                    else bracketHighlights.filterNot { br ->
                        val brLow = br.lowercase()
                        searchHighlights.any { sh ->
    ...
    override fun advance(amount: Float) {
        ...
        // If the search phrase or the exact-match toggle changed, redraw tabs + settings so the
        // gray-out and highlights update with the new mode.
        val cur = LunaSettingsUIModsPanel.currentSearch
        val curExact = LunaSettingsUIModsPanel.exactMatchOnly
        if (cur != lastObservedSearch || curExact != lastObservedExact) {
            lastObservedSearch = cur
            lastObservedExact = curExact
            if (selectedMod != null) {
                recreateTabs()
                recreatePanel()
            }
    ...
    private var lastObservedSearch: String = ""
    private var lastObservedExact: Boolean = false
