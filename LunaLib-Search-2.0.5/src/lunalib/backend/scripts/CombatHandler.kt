...
class CombatHandler : EveryFrameCombatPlugin
{
    ...
    // Drawn as a second line under "Version Checker" when there are updates. Rendered as a
    // separate string so each line can be horizontally centered on the button — the old
    // appendIndented call left-aligned the count under the main label.
    private var versionUpdateText: LazyFont.DrawableString? = null
    ...
    override fun init(engine: CombatEngineAPI?)
    {
        ...
        // Tooltip body keeps the small font (it's a multi-line paragraph and small is fine for
        // an explanatory popup). The two main-menu buttons use Orbitron — it's the same style
        // vanilla Starsector uses for button labels, and renders crisp at the button size
        // instead of the blurry / squished look DEFAULT_SMALL gave at 15px.
        val tipFont: LazyFont = LazyFont.loadFont(Fonts.DEFAULT_SMALL)
        val buttonFont: LazyFont = LazyFont.loadFont(Fonts.ORBITRON_20AABOLD)

        tip = tipFont.createText("LunaLib\n", Misc.getHighlightColor(), 15f)
        ...
        settingsButtonText = buttonFont.createText("Mod Settings", Misc.getBasePlayerColor(), 14f)

        versionButtonText = buttonFont.createText("Version Checker", Misc.getBasePlayerColor(), 14f)
        versionUpdateText = buttonFont.createText("", Misc.getHighlightColor(), 14f)
    }
    ...
    fun addVersionButton()
    {
        ...
        // Always keep the main label as a single line, then render the count (if any) on its own
        // string so it can be centered independently. The combined-string approach (text + "\n" +
        // appendIndented) left-aligned the second line within the wider text block.
        versionButtonText!!.text = "Version Checker"
        versionUpdateText!!.text = when {
            count == 1 -> "($count Update)"
            count != 0 -> "($count Updates)"
            else -> ""
        }

        val centerX = x + buttonWidth / 2
        if (versionUpdateText!!.text.isEmpty()) {
            // Single-line layout: vertically center the main label on the button.
            versionButtonText!!.draw(centerX - versionButtonText!!.width / 2f, (y + buttonHeight / 2) + versionButtonText!!.height / 2f)
        } else {
            // Two-line layout: place the main label in the upper half, the count in the lower.
            // Each is centered horizontally on its own width.
            val totalH = versionButtonText!!.height + versionUpdateText!!.height
            val topY = (y + buttonHeight / 2) + totalH / 2f
            versionButtonText!!.draw(centerX - versionButtonText!!.width / 2f, topY)
            versionUpdateText!!.draw(centerX - versionUpdateText!!.width / 2f, topY - versionButtonText!!.height)
        }
    }
